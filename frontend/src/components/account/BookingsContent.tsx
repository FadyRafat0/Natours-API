...
const BookingsContent = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        setLoading(true);
        const res = await API.get(`/bookings?user=${user?._id}&page=${page}&limit=${limit}`);
  ...
  if (loading) return <div style={{ fontSize: '1.6rem' }}>Loading bookings...</div>;
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            {bookings.map((booking) => (
              <div key={booking._id} style={{ display: 'flex', backgroundColor: '#fff', borderRadius: '1rem', overflow: 'hidden', boxShadow: '0 1.5rem 4rem rgba(0,0,0,0.1)' }}>
                <img src={tourImg(booking.tour.imageCover)} alt={booking.tour.name} style={{ width: '150px', objectFit: 'cover' }} />
                <div style={{ padding: '2rem', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  ...
                  <p style={{ fontSize: '1.4rem', color: '#777' }}>
                    <strong style={{ color: '#55c57a' }}>Price:</strong> ${booking.price} &bull; <strong style={{ color: '#55c57a' }}>Date Paid:</strong> {formatDate(booking.createdAt)}
                  </p>
                </div>
                <div style={{ padding: '2rem', display: 'flex', alignItems: 'center' }}>
                  <span className="btn btn--green btn--small" style={{ cursor: 'default' }}>{booking.paid ? 'PAID' : 'PENDING'}</span>
                </div>
              </div>
            ))}
          </div>
