...
const AdminTours = () => {
  ...
  const handleSave = async () => {
    try {
      if (editingTour) {
        await API.patch(`/tours/${editingTour._id}`, { name: formName, price: Number(formPrice), difficulty: formDifficulty });
      } else {
        await API.post(`/tours`, { name: formName, price: Number(formPrice), difficulty: formDifficulty, maxGroupSize: 10, duration: 5, summary: 'A great tour' });
      }
      setEditingTour(null);
      setCreating(false);
      fetchTours();
  ...
  return (
    <div className="user-view__form-container" style={{ maxWidth: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <button className="btn btn--small btn--green" onClick={openCreate}>Add Tour</button>
      </div>
            ...
            <tbody>
              {tours.map(t => (
                <tr key={t._id} style={{ borderBottom: '1px solid #f4f2f2' }}>
                  ...
                  <td style={{ padding: '1.5rem', textTransform: 'capitalize' }}>{t.difficulty}</td>
                  <td style={{ padding: '1.5rem', textAlign: 'right' }}>
                    <button className="btn-text" style={{ marginRight: '1rem' }} onClick={() => openEdit(t)}>Edit</button>
                    <button className="btn-text" style={{ color: '#eb4d4b' }} onClick={() => setDeletingId(t._id)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal isOpen={!!deletingId} onClose={() => setDeletingId(null)} title="Confirm Delete">
        <p style={{ fontSize: '1.6rem', marginBottom: '2rem' }}>Are you sure you want to delete this tour?</p>
      ...
      <Modal isOpen={creating || !!editingTour} onClose={() => { setCreating(false); setEditingTour(null); }} title={creating ? "Create Tour" : "Edit Tour"}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div>
            <label className="form__label">Name <span style={{color: 'red'}}>*</span></label>
            <input className="form__input" value={formName} onChange={e => setFormName(e.target.value)} required />
          </div>
          <div>
            <label className="form__label">Price <span style={{color: 'red'}}>*</span></label>
            <input className="form__input" type="number" value={formPrice} onChange={e => setFormPrice(e.target.value)} required />
          </div>
          <div>
            <label className="form__label">Difficulty <span style={{color: 'red'}}>*</span></label>
            <select className="form__input" value={formDifficulty} onChange={e => setFormDifficulty(e.target.value)}>
