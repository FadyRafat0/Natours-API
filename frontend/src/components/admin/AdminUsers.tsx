...
const AdminUsers = () => {
  ...
  const handleSave = async () => {
    try {
      if (editingUser) {
        await API.patch(`/users/${editingUser._id}`, { name: formName, email: formEmail, role: formRole });
      } else {
        await API.post(`/users`, { name: formName, email: formEmail, role: formRole, password: 'password123', passwordConfirm: 'password123' });
      }
      setEditingUser(null);
      setCreating(false);
      fetchUsers();
  ...
  return (
    <div className="user-view__form-container" style={{ maxWidth: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <button className="btn btn--small btn--green" onClick={openCreate}>Add User</button>
      </div>
              ...
              {users.map(u => (
                <tr key={u._id} style={{ borderBottom: '1px solid #f4f2f2' }}>
                  ...
                  <td style={{ padding: '1.5rem', textAlign: 'right' }}>
                    <button className="btn-text" style={{ marginRight: '1rem' }} onClick={() => openEdit(u)}>Edit</button>
                    <button className="btn-text" style={{ color: '#eb4d4b' }} onClick={() => setDeletingId(u._id)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal isOpen={!!deletingId} onClose={() => setDeletingId(null)} title="Confirm Delete">
        <p style={{ fontSize: '1.6rem', marginBottom: '2rem' }}>Are you sure you want to delete this user?</p>
      ...
      <Modal isOpen={creating || !!editingUser} onClose={() => { setCreating(false); setEditingUser(null); }} title={creating ? "Create User" : "Edit User"}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div>
            <label className="form__label">Email <span style={{color: 'red'}}>*</span></label>
            <input className="form__input" type="email" value={formEmail} onChange={e => setFormEmail(e.target.value)} required />
          </div>
          <div>
            <label className="form__label">Role <span style={{color: 'red'}}>*</span></label>
            <select className="form__input" value={formRole} onChange={e => setFormRole(e.target.value)}>
