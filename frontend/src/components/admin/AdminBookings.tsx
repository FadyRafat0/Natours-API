  ...
  return (
    <div className="user-view__form-container" style={{ maxWidth: '100%' }}>
            ...
            <tbody>
              {bookings.map(b => (
                <tr key={b._id} style={{ borderBottom: '1px solid #f4f2f2' }}>
                  <td style={{ padding: '1.5rem' }}>{b.tour?.name || 'Unknown'}</td>
                  <td style={{ padding: '1.5rem' }}>{b.user?.email || 'Unknown'}</td>
                  <td style={{ padding: '1.5rem' }}>${b.price}</td>
                  <td style={{ padding: '1.5rem', textAlign: 'right' }}>
                    <button className="btn-text" style={{ color: '#eb4d4b' }} onClick={() => setDeletingId(b._id)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal isOpen={!!deletingId} onClose={() => setDeletingId(null)} title="Confirm Delete">
        <p style={{ fontSize: '1.6rem', marginBottom: '2rem' }}>Are you sure you want to delete this booking?</p>
